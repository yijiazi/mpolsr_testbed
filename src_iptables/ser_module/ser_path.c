...
static int pth_copy_path(struct olsr_path* path, char* path_data, unsigned long* pSize, ip_v4_addr* nextHop, int src_local, struct ser_header* pheader)
{
    ...
    *pSize += sizeof(struct ser_header);
    ...
    DBG_HK(2,"find_datagram_path : path and first hop (%lX) ready for datagram update\n",*nextHop);
                        ...
                        ip_v4_addr dstAddr_req, ip_v4_addr* nextHop, struct ser_header* pheader)
{
    ...
    if(src_node == NULL)
    {
        DBG_HK(2,"find_datagram_path : failed to find src node (%lX) in TC data\n",srcAddr_req);
    ...
    if(dst_node == NULL)
    {
        DBG_HK(2,"find_datagram_path : failed to find dst node (%lX) in TC data\n",dstAddr_req);
        READ_UNLOCK(g_ser_tc_lock);
        return -1;
    }
    ...
    DBG_HK(2,"find_datagram_path : search for path from %lX to %lX\n", srcAddr, dstAddr);
    ...
    while(buffer)
    {
        if((buffer->src_addr_ip == srcAddr) && (buffer->dst_addr_ip == dstAddr))
        {
            ...
            DBG_HK(2,"find_datagram_path : path from %lX to %lX is in path cache\n", srcAddr, dstAddr);
    ...
    if(buffer != NULL)
    {
        ...
        DBG_HK(2,"find_datagram_path : no path from %lX to %lX after k_dijstra\n", srcAddr, dstAddr);
        return -1;
    }
    ...
    DBG_HK(2,"find_datagram_path : no path from %lX to %lX in path cache\n", srcAddr, dstAddr);
...
int pth_isEndOfPath(char* path, unsigned long size)
{
...
int pth_checkNextHop(char* path, unsigned long size, ip_v4_addr* nextHop)
{
    struct ser_header       header;
    ip_v4_addr              next_addr;
    ip_v4_addr              curr_addr;
    int                     offset;
    struct local_link*      link;
    ...
    if(cur >= hops)
    {
        MSG_FAILED("pth_checkNextHop :  invalid path information (cur hop >= #hpos)\n");
        return -1;  // can get next hop if reach end of path
    }

    // check local addr is current hop
    //--------------------------------
    offset = (cur-1)*ip_v4_size + sizeof(struct ser_header);
    memcpy(&curr_addr,path+offset,ip_v4_size);
    ...
    if(curr_addr != g_local_addr_tab[0])
    {
        ...
        MSG_FAILED("pth_checkNextHop :  current hop is not for this node !!!\n");
        READ_UNLOCK(g_ser_tc_lock);
        return -1;
    }


    // get next addr and update path infos (ie current hop)
    //-----------------------------------------------------
    cur++;
    header.cur = htons(cur);

    offset += ip_v4_size;
    memcpy(path ,&header,sizeof(struct ser_header));             // update path infos
    memcpy(&next_addr,path+offset,ip_v4_size);                   // get next addr
