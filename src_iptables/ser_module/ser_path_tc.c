...
#ifndef APP_TEST
    rwlock_t                g_ser_tc_lock = __RW_LOCK_UNLOCKED(g_ser_tc_lock);
#else
...
void tc_add_alias(ip_v4_addr addr_ip, ip_v4_addr addr_alias)
{
    ...
    if(node == NULL)
    {
        MSG_FAILED("Failed to add node alias : %lX -> %lX\n", addr_ip,addr_alias);
...
void tc_remove_alias(ip_v4_addr addr_ip, ip_v4_addr addr_alias)
{
    ...
    if(node == NULL)
    {
        MSG_FAILED("Failed to remove node alias : %lX -> %lX\n", addr_ip,addr_alias);
